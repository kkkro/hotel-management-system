...
namespace WebKhachSan.Controllers
{
    ...
    public class ThuePhongController : Controller
    {
        ...
        private async Task ValidateOfflineRentalAsync(ThuePhongOfflineViewModel model)
        {
            ...
            {
                ModelState.AddModelError(nameof(model.NgayTra), "Vui long chon ngay tra phong.");
            }
            else if (model.NgayNhan.HasValue && model.NgayTra.Value.Date < model.NgayNhan.Value.Date)
            {
